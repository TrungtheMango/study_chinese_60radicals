...
function loadProgress() {
  try {
    ...
    // Backward-compatible: nếu user đã có dữ liệu cũ không có soundOn
    if (parsed?.settings && typeof parsed.settings.soundOn === "undefined") {
      parsed.settings.soundOn = true;
    }
...
// NEW: audio element with fallback ogg -> mp3
function makeAudio(baseName: string): HTMLAudioElement {
  const a = document.createElement("audio");
  a.preload = "auto";

  const s1 = document.createElement("source");
  s1.src = `/sounds/${baseName}.ogg`;
  s1.type = "audio/ogg";

  const s2 = document.createElement("source");
  s2.src = `/sounds/${baseName}.mp3`;
  s2.type = "audio/mpeg";
      ...
      <DialogContent className="rounded-2xl">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3">
            <div>
              <div className="text-sm font-semibold">Âm thanh</div>
              <div className="text-xs text-slate-500">Bật/tắt toàn bộ sound effect.</div>
            </div>
            <Switch checked={s.soundOn} onCheckedChange={(v) => patchSettings({ soundOn: v })} />
          </div>
...
function StudySession({ api, mode, onExit }: { api: UseProgressReturn; mode: ModeType; onExit: () => void }) {
  ...
  // NEW: SOUND FX refs (fallback ogg -> mp3)
  const sBackRef = useRef<HTMLAudioElement | null>(null);
  const sCorrectRef = useRef<HTMLAudioElement | null>(null);
  const sFinishRef = useRef<HTMLAudioElement | null>(null);
  const sFlipRef = useRef<HTMLAudioElement | null>(null);
  const sNextRef = useRef<HTMLAudioElement | null>(null);
  const sSelectRef = useRef<HTMLAudioElement | null>(null);
  const sWrongRef = useRef<HTMLAudioElement | null>(null);

  const playSfx = async (ref: React.MutableRefObject<HTMLAudioElement | null>) => {
    ...
    try {
      ...
      // ignore (browser may block/codec)
    }
  };

  useEffect(() => {
    // init 1 lần khi vào session
    sBackRef.current = makeAudio("back");
    sCorrectRef.current = makeAudio("correct");
    sFinishRef.current = makeAudio("finish");
    sFlipRef.current = makeAudio("flip");
    sNextRef.current = makeAudio("next");
    sSelectRef.current = makeAudio("select");
    sWrongRef.current = makeAudio("wrong");

    [sBackRef, sCorrectRef, sFinishRef, sFlipRef, sNextRef, sSelectRef, sWrongRef].forEach((r) => {
      if (!r.current) return;
      r.current.volume = 0.6;
  ...
  const goNextIndex = () => {
    const willWrap = index + 1 >= deck.length;
    if (willWrap) playSfx(sFinishRef);
    else playSfx(sNextRef);
  ...
  const grade = (g: number) => {
    ...
    // sound for grading (learn/review only)
    if (mode !== "quiz") {
      playSfx(g >= 3 ? sCorrectRef : sWrongRef);
    }
  ...
  }, [settings.keyboardShortcuts, flipped, mode, card.id, index, deck.length]);
  ...
  const submitQuiz = () => {
    ...
    playSfx(ok ? sCorrectRef : sWrongRef);
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Button
            variant="secondary"
            className="rounded-2xl gap-2 bg-white/70 hover:bg-white"
            onClick={() => {
              playSfx(sBackRef);
              onExit?.();
            }}
                        ...
                        return (
                          <button
                            ...
                            className={
                              ...
                              (picked && !quizReveal ? "border-sky-300 bg-sky-50 ring-1 ring-sky-200" : "border-slate-200 bg-white") +
                              (correct ? " border-emerald-200 bg-emerald-50 ring-1 ring-emerald-200" : "") +
                              (wrong ? " border-rose-200 bg-rose-50 ring-1 ring-rose-200" : "")
                            }
                          >
              ...
              <div className="mt-4">
                {!flipped ? (
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      className={`rounded-2xl ${theme.solid}`}
                      onClick={() => {
                        playSfx(sFlipRef);
                        setFlipped(true);
                      }}
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        <Input
          ...
          placeholder="Tìm… (vd: 氵, shui, nước, 路)"
            ...
            {list.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className={
                    "rounded-2xl border border-slate-200 bg-white p-3 " + (pr.learned ? "ring-1 ring-emerald-100" : "")
                  }
                >
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 text-slate-900">
      <div className="mx-auto max-w-6xl px-4 py-6">
                ...
                <CardContent className="space-y-3">
                  <div className="grid gap-3 md:grid-cols-3">
                    ...
                    >
                      ...
                      <div className="mt-3 flex items-center gap-2 text-sm">
                        <Badge className="rounded-2xl border border-amber-200 bg-amber-50 text-amber-800">
                          Đến hạn: {dueCount}
                        </Badge>
                        <ChevronRight className="h-4 w-4 text-slate-500" />
                      </div>
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-white/70 p-4 text-sm text-slate-600">
          Created by <span className="font-semibold text-slate-900">TrungtheMango</span> • IG:{" "}
          <span className="font-semibold text-slate-900">trung_the_mango</span>
        </div>
      </div>
    </div>
  );
